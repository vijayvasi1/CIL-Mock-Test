...
<body>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      ...
      {downloadNotice && (
        <div className="bg-emerald-500/20 border-2 border-emerald-500 text-emerald-950 px-4 py-3 rounded-xl flex items-center justify-between text-xs sm:text-sm font-bold shadow-md animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="p-3.5 bg-white border border-slate-200 hover:border-amber-500 rounded-xl shadow-xs transition-all flex items-start justify-between group"
        >
          <div>
            <span className="text-[10px] font-extrabold uppercase text-amber-600 tracking-wider">Official Noticeboard</span>
            <h4 className="text-xs font-bold text-slate-900 group-hover:text-amber-600 mt-0.5">
              CIL Career & MT Recruitment
            </h4>
            <p className="text-[11px] text-slate-500 mt-0.5">Latest official bulletins & updates</p>
          </div>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="p-3.5 bg-white border border-slate-200 hover:border-emerald-500 rounded-xl shadow-xs transition-all flex items-start justify-between group"
        >
          <div>
            <span className="text-[10px] font-extrabold uppercase text-emerald-600 tracking-wider">Govt of India</span>
            <h4 className="text-xs font-bold text-slate-900 group-hover:text-emerald-600 mt-0.5">
              Ministry of Coal Statistics
            </h4>
            <p className="text-[11px] text-slate-500 mt-0.5">Annual reports & production KPIs</p>
          </div>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="p-3.5 bg-white border border-slate-200 hover:border-indigo-500 rounded-xl shadow-xs transition-all flex items-start justify-between group"
        >
          <div>
            <span className="text-[10px] font-extrabold uppercase text-indigo-600 tracking-wider">Technical Wing</span>
            <h4 className="text-xs font-bold text-slate-900 group-hover:text-indigo-600 mt-0.5">
              CMPDIL Exploration Portal
            </h4>
            <p className="text-[11px] text-slate-500 mt-0.5">Mine planning & IT architecture</p>
          </div>
        ...
        <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-extrabold uppercase text-amber-800 tracking-wider">Offline Access</div>
            <div className="text-xs font-bold text-slate-900">100% Offline Ready</div>
            <div className="text-[11px] text-slate-600">Export as HTML/PDF</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredResources.map((item) => (
          <div
            key={item.id}
            className="bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-5 shadow-sm flex flex-col justify-between space-y-4 transition-all"
          >
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
              <button
                ...
                className="flex-1 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs flex items-center justify-center gap-1.5 transition-all"
              >
                <BookOpen className="w-3.5 h-3.5" />
              ...
              <button
                ...
                className="p-2 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 rounded-xl border border-slate-200 transition-all"
      ...
      {activeReadingItem && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl max-w-4xl w-full max-h-[90vh] shadow-2xl flex flex-col justify-between overflow-hidden animate-fade-in">
            {/* Modal Header */}
            ...
            <div className="p-6 sm:p-8 overflow-y-auto space-y-6 text-slate-800 leading-relaxed max-h-[70vh] scrollbar-thin">
              {/* Highlights Callout */}
              ...
              <div className="prose prose-slate max-w-none text-xs sm:text-sm leading-relaxed space-y-4">
                {activeReadingItem.contentMarkdown?.split("\n\n").map((para, i) => {
                  if (para.startsWith("# ")) {
                    return (
                      <h2 key={i} className="text-xl font-extrabold text-slate-900 border-b border-slate-200 pb-2">
                        {para.replace("# ", "")}
                      </h2>
                    );
                  }
                  if (para.startsWith("## ")) {
                    return (
                      <h3 key={i} className="text-base font-bold text-slate-900 text-amber-700 mt-4">
                        {para.replace("## ", "")}
                      </h3>
                    );
                  }
                  if (para.startsWith("### ")) {
                    return (
                      <h4 key={i} className="text-sm font-bold text-slate-800 mt-2">
                        {para.replace("### ", "")}
                      </h4>
                    );
                  }
                  if (para.startsWith("| ")) {
                    ...
                    return (
                      <div key={i} className="overflow-x-auto my-3 border border-slate-200 rounded-lg">
